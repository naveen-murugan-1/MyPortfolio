...
const Experience = () => {
  ...
  const ExperienceCard = ({ exp, index }) => (
    <motion.div
      ...
      className="bg-gray-950 p-6 rounded-xl transition-all duration-300 border border-gray-700/50 flex flex-col w-full md:max-w-md h-auto focus:outline-none focus:ring-2 focus:ring-violet-500 hover:scale-[1.02] hover:border-violet-500/50 hover:shadow-[0_0_20px_rgba(139,92,246,0.25)]"
    >
      <div className="flex justify-between items-start mb-4 gap-3">
        <div className="flex gap-3 items-center">
          {exp.companyLogo && (
            <img src={exp.companyLogo} alt={exp.company} className="w-12 h-12 rounded-lg object-contain bg-black p-1 border border-gray-700" />
          )}
          <div>
      ...
      <ul className="text-gray-300 mt-2 space-y-1 flex-grow">
        {exp.description && exp.description.slice(0, 3).map((point, idx) => (
          <li key={idx} className="flex items-start">
            <span className="text-violet-400 mr-2 font-black">•</span>
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 ${bgImage ? 'bg-black/60' : 'bg-black/90'}`}></div>
      <Doodles sectionName="experience" />
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full flex flex-col items-center">
        <div className="section-heading-container mb-12">
          <h2 className="section-heading">
            <span className="text-white capitalize">Experience</span>
          </h2>
