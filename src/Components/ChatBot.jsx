...
const ChatBot = () => {
    ...
    const processResponse = async (userInput) => {
        ...
        // 1. Direct Entity Matching (Fuzzy word and description logic)
        ...
        const projectMatch = findMatch(contextData.projects, 'title');
        const certificateMatch = findMatch(contextData.certificates, 'title');
        const skillMatch = findMatch(contextData.skills, 'name');
        const expMatch = findMatch(contextData.experience, 'company') || findMatch(contextData.experience, 'role');
        const achMatch = findMatch(contextData.achievements, 'title');
        ...
        if (projectMatch) {
            ...
            responseObj = {
                reply: `I hold a certification in ${certificateMatch.title} from ${certificateMatch.issuer || certificateMatch.platform || "an external provider"}. ${certificateMatch.description || ""}`,
                action: "certificates",
        ...
        // 4. Bio / Personal Info queries
        else if (lowerInput.includes('who is naveen') || lowerInput.includes('tell me about naveen') || lowerInput.includes('describe naveen') || lowerInput.includes('about you') || lowerInput.includes('about yourself') || lowerInput === "naveen") {
             ...
             responseObj = {
                 reply: `I am ${name}, a ${role}. ${bio} I have completed ${contextData.projects?.length || 0} notable projects.`,
                 action: "hero",
                 highlight: ""
             };
        }
        ...
        // Keyword Match Fallbacks
            ...
            const projNames = contextData.projects?.slice(0, 2).map(p => p.title).join(" and ") || "various modern applications";
            responseObj = {
                reply: `Alright… let me take you through my work. Top projects in my portfolio include ${projNames}.`,
                action: "projects",
                highlight: ""
            };
        }
        // 3. Navigation intents
            ...
            const topSkills = contextData.skills?.slice(0, 4).map(s => s.name).join(", ") || "my entire stack";
            responseObj = {
                reply: `Let me show you my skills section. I’ve built deep expertise in ${topSkills}.`,
                action: "skills",
                highlight: ""
            };
        }
            ...
            const eduText = edu ? `I completed my ${edu.degree} at ${edu.college}.` : `I completed my degree with a strong foundation in computer science.`;
            responseObj = {
                reply: `I’ve taken you to my education section. ${eduText}`,
                action: "education",
                highlight: ""
            };
        }
            ...
            const role = contextData.experience?.[0];
            const roleText = role ? `I have professional experience working as a ${role.role} at ${role.company}.` : "I’ve worked on various responsibilities that sharpened my software engineering skills.";
            responseObj = {
                reply: `Let me show you my experience. ${roleText}`,
                action: "experience",
                highlight: ""
            };
        }
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 font-sans">
            {isOpen && (
                <div className="bg-black border border-violet-500/30 w-80 md:w-96 h-[500px] rounded-2xl  flex flex-col overflow-hidden mb-4 animate-fade-in-up backdrop-blur-md">
                    <div className="bg-gray-950 p-4 flex justify-between items-center border-b border-gray-700">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                 <h3 className="text-white font-bold">Naveen</h3>
                                <p className="text-xs text-violet-400">Online • Portfolio Guide</p>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-black/90 scrollbar-thin scrollbar-thumb-gray-700">
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-[80%] p-3 rounded-lg text-sm ${msg.isBot
                                    ? 'bg-gray-950 text-gray-200 rounded-tl-none border border-gray-700'
                    ...
                    <div className="p-4 bg-gray-950 border-t border-gray-700">
                        {isListening && <p className="text-xs text-violet-400 mb-2 animate-pulse">Listening...</p>}
                        <div className="flex items-center gap-2 bg-black rounded-lg p-1 border border-gray-700 focus-within:border-violet-500 transition-colors">
                            <input
                                type="text"
                                value={input}
                            ...
                            />
                             <button onClick={() => handleSend()} className="p-2 bg-violet-600 rounded-md text-white hover:bg-violet-500 transition-colors">
                                <Send size={16} />
                            </button>
                        </div>
                    </div>
            ...
            {!isOpen && (
                <button
                    ...
                    className="bg-violet-600 hover:bg-violet-500 text-white p-4 rounded-full transition-all transform hover:scale-110 flex items-center gap-2 group"
                >
