...
const Contact = () => {
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 ${bgImage ? 'bg-black/60' : 'bg-black/90'} z-0`}></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full flex flex-col items-center">
        ...
        >
          <h2 className="section-heading">
            <span className="text-white capitalize">Contact Me</span>
          </h2>
        ...
        <div className="flex flex-col lg:flex-row gap-12 w-full max-w-5xl">
          ...
          <motion.div 
            ...
            className="lg:w-1/2 bg-black/40 backdrop-blur-sm p-6 rounded-2xl border border-gray-800"
          >
            <h3 className="text-2xl font-bold mb-6 text-violet-400 tracking-wide">Get in Touch</h3>
              ...
              {(() => {
                ...
                return allItems.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-violet-400 mr-4 transition-transform group-hover:scale-110">
                        {getIcon(item.platform)}
                      </div>
                      <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-gray-200 group-hover:text-violet-400 transition-colors text-sm font-medium">
                        {item.label}
                      </a>
                    </motion.div>
                  );
          ...
          <motion.div 
            ...
            className="lg:w-1/2 bg-black/40 backdrop-blur-sm p-6 rounded-2xl border border-gray-800"
          >
            <h3 className="text-2xl font-bold mb-6 text-violet-400 tracking-wide">Send a Message</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-950/80 border border-gray-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 transition-all font-sans"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-gray-950/80 border border-gray-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 transition-all font-sans resize-none"
                ></textarea>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full bg-violet-600 hover:bg-violet-500 text-white font-bold py-2.5 rounded-lg transition-all disabled:opacity-60 text-sm "
              >
                {isSubmitting ? 'Sending...' : 'Send Message'}
              </motion.button>

              {submitStatus === 'success' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 p-3 bg-violet-500/10 border border-violet-500/30 text-violet-400 rounded-lg text-xs font-semibold text-center">
