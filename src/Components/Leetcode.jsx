    ...
    const fetchLeetCodeData = async () => {
      try {
        const [response, aboutRes] = await Promise.all([
          api.get('/leetcode/Naveen_Murugan_1'),
          api.get('/about')
        ]);
    ...
    return (
      <section className="py-24 bg-[#0a0a0a] flex justify-center items-center min-h-[500px]">
        <div className="relative">
          ...
          <Loader className="relative z-10 animate-spin text-violet-400" size={64} />
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="py-24 bg-[#0a0a0a] flex justify-center items-center">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-8 text-center max-w-lg">
          <h3 className="text-xl text-white font-bold mb-3">Unable to Load Stats</h3>
          <p className="text-gray-400 mb-6">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="px-6 py-2 bg-violet-500/20 hover:bg-violet-500/30 text-violet-400 rounded-lg transition-colors border border-violet-500/30"
          >
            Retry
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="section-heading-container mb-16">
          <h2 className="section-heading">
            <span className="text-white capitalize">LeetCode</span>
          </h2>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
          ...
          <a
            href="https://leetcode.com/u/Naveen_Murugan_1/"
            target="_blank"
            rel="noopener noreferrer"
            className="group flex items-center gap-3 px-6 py-3 bg-black border border-gray-800 rounded-xl hover:border-violet-500/50 hover:bg-gray-950 transition-all duration-300"
          >
            <span className="text-gray-300 group-hover:text-white font-medium">View Profile</span>
            <ExternalLink size={18} className="text-gray-500 group-hover:text-violet-400 transition-colors" />
          </a>
        </div>

        <div className={`grid grid-cols-1 ${badges.length > 0 ? 'lg:grid-cols-12' : 'max-w-4xl mx-auto'} gap-8`}>
          {/* Main Stats Column */}
          <div className={`${badges.length > 0 ? 'lg:col-span-4' : 'w-full'} space-y-6`}>
            {/* Total Solved Card */}
            <div className="bg-black/50 backdrop-blur-xl rounded-3xl p-8 border border-gray-800 relative overflow-hidden group hover:border-violet-500/30 transition-all duration-300">
              <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                <Trophy size={100} className="text-violet-500 transform rotate-12" />
              </div>

              <div className="relative z-10">
                <p className="text-violet-400 font-medium mb-2">Total Solved</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-black/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-800 hover:border-violet-500/30 transition-all duration-300">
          ...
          {badges.length > 0 && (
            <div className="lg:col-span-8">
              <div className="bg-black/50 backdrop-blur-xl rounded-3xl p-8 border border-gray-800 h-full hover:border-violet-500/30 transition-all duration-300">
                <div className="flex items-center gap-3 mb-8">
                  <Award className="text-violet-400" size={24} />
                  <h3 className="text-xl font-bold text-violet-400">Badges & Achievements</h3>
                  <span className="px-3 py-1 bg-gray-950 rounded-full text-xs text-gray-400 border border-gray-700">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4">
                  {badges.map((badge, index) => (
                    <div
                      key={index}
                      className="group flex flex-col items-center justify-center p-6 bg-black rounded-2xl border border-gray-800 hover:border-violet-500/50 transition-all duration-300 hover:-translate-y-1"
                    >
